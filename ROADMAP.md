...
*Online fleet door (LANDED 2026-08-31):* the canvas model picker can now select a **local** GGUF (resident engine, 🔵) or a **cloud** model (🟣) per agent run. Cloud models come from live `/v1/models` catalogs fetched per provider (Groq, Gemini, DeepSeek, OpenRouter, NIM, OpenCode Go/Zen) and grouped in the picker with their free-tier quota note; free providers preferred (direct free keys beat the shared OpenRouter pool for volume). Gemini quirk: Google's OpenAI-compatible endpoint has no `/models` route for unauthenticated callers (404), so `deck-agents::providers` falls back to a static current-generation list instead of surfacing the error; with a key stored the live catalog supersedes it (verified live: 53 models). The OpenCode rewrite writes `apiKey: {env:<PROVIDER>_API_KEY}` — an env *reference*, never a secret — bridging the keychain-first convention to the harness, which cannot read the OS keychain; verified end-to-end: keychain → binding → opencode → Gemini 3.6 generation (Google retired the 1.5/2.5 family for new keys; the static list tracks the current generation). `deck agents` (CLI) and the Tauri `agents_fleet/agents_catalog/agents_use/agents_quota_set` commands are the two doors; `deck-agents` (`model`/`providers`/`quota`/`rewrite` + `ops`) is the domain crate. **Anti-reinvent decision:** the harness-config *routing* (Goose/OpenCode/DeepSeek↔provider rewiring) is intentionally NOT rebuilt here — it's delegated to existing OSS (AgentO, oneharness, universal-agent-config); `deck-agents` keeps only the cyberdeck-unique pieces: the free-tier provider catalog, quota tracking, and live model catalogs. DeepSeek/Goose harness config-rewriting stays stubbed until those CLIs are installed on a machine with real configs to inspect.

*Provider key storage (LANDED 2026-08-31):* cloud API keys live in the **OS keychain** (`deck-agents::keys` via the `keyring` crate → Secret Service on Linux / Keychain on macOS / Credential Manager on Windows) — the industry-standard store, **no app-level obfuscation or master passphrase** (both are the non-standard, security-weakening pattern). Resolution is keychain-first, `<PROVIDER>_API_KEY` env fallback (matching `gh`). Doors: `deck secrets {list,set,unset,check}` (set reads stdin so keys never hit argv/history) and Tauri `secret_list/set/unset/check` with a provider-key ●/○ strip in the Workspace advanced controls + picker. Keys never touch the repo, settings KV, or logs; only names + masked previews cross the UI. Linux note: Secret Service must report an unlocked default collection — on this host the default silently pointed at a locked `Default_keyring`; fixed by aliasing the `default` alias to the unlocked `login` collection (reversible, preserves the 3 pre-existing items).**

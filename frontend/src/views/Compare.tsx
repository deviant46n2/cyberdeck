...
export default function Compare() {
  ...
  const refresh = () => {
    setReport(null);
    setLoading(true);
    api.compareRun({ model: '', engines: [] as string[], ollama: [] as string[], tasks: tasks, maxTokens, runs: tasks.length, seed }).then(setReport).catch(() => {}).finally(() => setLoading(false));
  };
  ...
  const handleTasksChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setTasks(e.target.value.split(",").map((s: string) => s.trim()).filter((s: string) => s.length > 0));
  };
  ...
  // Single root element
  const titleEl = <div className="view-title">COMPARE</div>;
  const subtitle = report ? (
    <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 8 }}>
      <strong>Tasks:</strong> {report.tasks.join(", ")}
    </div>
  ) : (
        ...
        <div style={{ padding: 24, maxHeight: 400 }}>
          ...
          <div style={{ marginTop: 16, display: "grid", gap: 16 }}>
            ...
            <div>
              ...
              <input type="text" value={tasks.join(",")} onChange={handleTasksChange} placeholder="task1,task2" style={{ width: 200 }} />
              <span className="dim" style={{ fontSize: 10 }}>(comma-separated, runs per task default to 5)</span>
            </div>
            <div>
